    ...
    <a-tab-pane tab="投资计划上报" :key="refKeys[0]" :forceRender="true" :class="'jeecg-tabs-top'" :animated="false">
         <a-form-model ref="form" :model="model" :rules="validatorRules">
           <a-row>
                  ...
                  <a-col :xs="24" :sm="12">
                    <a-form-model-item label="本级投资总资金" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="currentAmount">
                      <a-input-number v-model="model.currentAmount" placeholder="请输入本级投资总资金" style="width: 100%" />
                    </a-form-model-item>
                  </a-col>
                  <a-col :xs="24" :sm="12">
                    <a-form-model-item label="下级投资总资金" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="lowerAmount">
 ...
 data() {
   return {
    open:false,
     ...
     refKeys: ['investPlanReport','investPlanRelationDetail', 'investPlanSubRelationDetail', ],
     tableKeys:['investPlanRelationDetail', 'investPlanSubRelationDetail', ],
  ...
  panelChange(val){
   ...
   editAfter() {
     ...
     if (this.model.id) {
       ...
       this.requestSubTableData(this.url.investPlanSubRelationDetail.list, params, this.investPlanSubRelationDetailTable)
     }
   },
   //校验所有一对一子表表单
