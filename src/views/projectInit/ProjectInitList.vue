  ...
  <div class="FixedAssetsInvestmentList">
    ...
    <a-card :bordered="false">
      ...
      <div class="table-operator">
        ...
        <a-button @click="handleAdd" type="primary" icon="plus">选择项目</a-button>
      ...
      <fixedAssetsInvestment-modal ref="modalForm" @ok="modalFormOk" :selectedRows="selectedRows"></fixedAssetsInvestment-modal>
    </a-card>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        changeStg:'/investment/fixedAssetsInvestment/initiation',
        invList:'/investment/fixedAssetsInvestment/list',
        tree: '/sys/sysDepart/queryTreeList',
        list: '/investment/fixedAssetsInvestment/listInitiation',
  ...
  methods: {
    handleAdd(){
      this.getList();
      this.ProjectInitList = []
      this.visibleShowRelated = true;
    },
    async handleOkRelation() {
      // let projectId = this.selectedRowKeys.join()
      // console.log(projectId,'projectId')
      const {id} = this.selectedRows[0]
      let result = await axios.post(this.url.changeStg + '?projectId=' + id)
      this.visibleShowRelated = false
      this.loadData();
    },
    async getList() {
      let result = await axios.get(this.url.invList)
